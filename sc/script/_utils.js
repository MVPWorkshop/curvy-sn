...
export const declareAllContracts = async (account) => {
  ...
  for (const file of allContracts) {
    if (file.endsWith("compiled_contract_class.json")) {
      const contract = json.parse(
        fs
          .readFileSync(path.join(directoryPath, file.replace("compiled_", "")))
          .toString("ascii")
      );
      const casm = json.parse(
        fs.readFileSync(path.join(directoryPath, file)).toString("ascii")
      );
  ...
  const curvyAccount = rawArtifacts.filter(
    (e) => e.name == "curvy_account_AccountV0"
  )[0];
...
export const deployProtocol = async (account, artifacts) => {
  ...
  console.log("Deployed contracts", deployResponse);
