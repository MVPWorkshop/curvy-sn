...
export class ContractListener extends EventEmitter {
  ...
  private async fetchEvents(): Promise<{
    ...
    if (latestBlock <= this.lastBlockScanned)
  ...
  private async processEvent(event: ContractEvent): Promise<{
    ...
    try {
      const contractAddress = validateAndParseAddress(event.from_address);
      const validContractAddress = validateAndParseAddress(
      ...
      if (
        validContractAddress ==
        validateAndParseAddress(myCalls[0].contractAddress)
      ) {
        rawData = myCalls[0].calldata;
      } else if (
        validContractAddress == validateAndParseAddress(myCalls[0].calldata[8])
      ) {
